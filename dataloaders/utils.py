...
def distance_map(mask, pad=10, v=0.15, relax=False):
    bounds = (0, 0, mask.shape[1] - 1, mask.shape[0] - 1)   # 图片尺寸

    bbox = get_bbox(mask, pad=pad, relax=relax)  # get image bounding box
    if bbox is not None:
        bbox = add_turbulence(bbox, v=v)    # bounding box随机放大
    else:
...
def compute_dismap(dismap, bbox):
    ...
    cv2.rectangle(dismap,(x_min, y_min), (x_max, y_max), color=255, thickness=-1)
...
def add_turbulence(bbox, v=0.15):
    ...
    '''
    x_min, y_min, x_max, y_max = bbox[:]
    x_min_new = int(x_min - v * abs(np.random.normal(0, 1)) * (x_max - x_min))
    x_max_new = int(x_max + v * abs(np.random.normal(0, 1)) * (x_max - x_min))
    y_min_new = int(y_min - v * abs(np.random.normal(0, 1)) * (y_max - y_min))
    y_max_new = int(y_max + v * abs(np.random.normal(0, 1)) * (y_max - y_min))
...
def get_bbox(mask, points=None, pad=0, relax=False):
    ...
    Returns: 紧贴（或者放大pad后）object的矩形框坐标，即bounding box坐标

    '''
    if points is not None:
        inds = np.flip(points.transpose(), axis=0)
    else:
        inds = np.where(mask > 0)   # 返回不等于0的坐标，([行]，[列])
