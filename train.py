...
from dataloaders import pascal_voc_aug, sbd, pascal
from networks import deeplab_resnet as resnet
from layers.loss import class_balanced_cross_entropy_loss, error_rate
...
if resume_epoch != nEpochs:
    ...
    composed_transforms_tr = transforms.Compose([
        ...
        tr.ConcatInputs(elems=('image', 'distance_map', 'fix')),
        tr.ToTensor()])
    ...
    pascal_voc_train = pascal_voc_aug.PascalVocAugDataset(split='train', transform=composed_transforms_tr)
    pascal_voc_val = pascal_voc_aug.PascalVocAugDataset(split='val', transform=composed_transforms_ts)

    if use_sbd:
        sbd_train = sbd.SBDSegmentation(split=['train', 'val'], transform=composed_transforms_tr, retname=True)
        db_train = combine_dbs([pascal_voc_train, sbd_train], excluded=[pascal_voc_val])
    else:
        db_train = pascal_voc_train
    ...
    testloader = DataLoader(pascal_voc_val, batch_size=testBatch, shuffle=False)
    ...
    for epoch in range(resume_epoch, nEpochs):
        ...
        if useTest and epoch % nTestInterval == (nTestInterval - 1):
            ...
            for ii, sample_batched in enumerate(testloader):
                inputs, gts, mask = sample_batched['concat'], sample_batched['gt'], sample_batched['distance_map']
                ...
                er_a = error_rate(output, gts, over_mask=False)
                er_m = error_rate(output, gts, over_mask=True, mask=mask)
