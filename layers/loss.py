...
def error_rate(output, gts, over_mask=False, mask=None):
    if over_mask:
        assert mask is not None
        mask[mask > 0] = 1
        output = output*mask
        gts = gts*mask
        er = len(torch.where(output != gts)[0]) / len(torch.where(mask != 0)[0])
    else:
        er = len(torch.where(output != gts)[0]) / output.numel()
    return er
